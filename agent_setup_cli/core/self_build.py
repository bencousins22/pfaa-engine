"""
Aussie Agents Self-Builder — The engine that builds itself.
...
class SelfBuilder:
    ...
    Workflow:
        1. introspect()  — Analyze own codebase with Aussie Agents tools
    ...
    async def introspect(self) -> SelfAnalysis:
        """Analyze the Aussie Agents engine's own source code using its own tools."""
    ...
    async def diagnose(self) -> list[str]:
        ...
        prompt = f"""You are reviewing Aussie Agents (Phase-Fluid Agent Architecture) — a Python 3.15
...
Focus on what would make this agent MORE POWERFUL at building itself."""
    ...
    async def propose_tool(self, description: str) -> dict[str, Any]:
        """
        Use Claude to generate a new Aussie Agents tool based on a description.
        ...
        prompt = f"""Generate a new Aussie Agents tool following this EXACT pattern from our codebase:
...
'''
    ...
    async def build_self(
        ...
        """
        print("╔══════════════════════════════════════════════════════╗")
        print("║  AUSSIE AGENTS SELF-BUILD CYCLE                     ║")
